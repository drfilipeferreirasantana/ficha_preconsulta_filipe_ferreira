...
async function syncToGoogle(task, { isHearing }) {
  if (!(await googleCalendar.isConnected())) return { id: null, link: null };
...
router.post('/', async (req, res) => {
  ...
  const task = await db.get('SELECT tasks.*, clients.name AS client_name, clients.phone AS client_phone FROM tasks LEFT JOIN clients ON clients.id = tasks.client_id WHERE tasks.id = ?', [info.lastInsertRowid]);
  ...
  res.status(201).json({ task, whatsapp_link: whatsappLink, google_connected: await googleCalendar.isConnected() });
});
...
router.put('/:id', async (req, res) => {
  ...
  await db.run(`
    UPDATE tasks SET title=:title, description=:description, assigned_to=:assigned_to,
      ...
      google_event_id=:google_event_id, google_event_link=:google_event_link
    WHERE id=:id
  `, { ...b, is_hearing: b.is_hearing ? 1 : 0, notify_client: b.notify_client ? 1 : 0 });
  res.json(await db.get('SELECT * FROM tasks WHERE id = ?', [req.params.id]));
});

router.delete('/:id', async (req, res) => {
  const existing = await db.get('SELECT * FROM tasks WHERE id = ?', [req.params.id]);
  if (existing && existing.google_event_id && await googleCalendar.isConnected()) {
    await googleCalendar.deleteEvent(existing.google_event_id);
  }
