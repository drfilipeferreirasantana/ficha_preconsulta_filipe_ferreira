...
router.get('/', async (req, res) => {
  ...
  if (upcoming) { sql += ' AND processes.next_deadline IS NOT NULL AND processes.next_deadline <= DATE_ADD(CURDATE(), INTERVAL ? DAY)'; params.push(Number(upcoming) || 7); }
  if (q) {
...
router.get('/:id', async (req, res) => {
  ...
  const updates = await db.all('SELECT * FROM process_updates WHERE process_id = ? ORDER BY date DESC', [process.id]);
  const finance = await db.all('SELECT * FROM finance_entries WHERE process_id = ? ORDER BY due_date', [process.id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    result = await db.transaction(async (tx) => {
      ...
      const processInfo = await tx.run(`
      ...
      const insertFinance = (data) => tx.run(`
      ...
      if (b.down_payment && Number(b.down_payment) > 0) {
        const info = await insertFinance({
  ...
  const client = await db.get('SELECT * FROM clients WHERE id = ?', [result.clientId]);
  const process = await db.get('SELECT * FROM processes WHERE id = ?', [result.processId]);
  const finance = result.financeInserted.length
    ? await db.all('SELECT * FROM finance_entries WHERE id IN (?)', [result.financeInserted.map((f) => f.id)])
    : [];
...
router.put('/:id', async (req, res) => {
  const existing = await db.get('SELECT * FROM processes WHERE id = ?', [req.params.id]);
  ...
  await db.run(`
    ...
    WHERE id=:id
  `, b);
  res.json(await db.get('SELECT * FROM processes WHERE id = ?', [req.params.id]));
});

router.delete('/:id', async (req, res) => {
  await db.run('DELETE FROM processes WHERE id = ?', [req.params.id]);
  res.status(204).end();
});
...
router.post('/:id/updates', async (req, res) => {
  ...
  const info = await db.run(`
    INSERT INTO process_updates (process_id, description, date, source) VALUES (?, ?, COALESCE(?, NOW()), ?)
  `, [req.params.id, description, date || null, source || 'manual']);
  ...
  res.status(201).json({
    update: await db.get('SELECT * FROM process_updates WHERE id = ?', [info.lastInsertRowid]),
